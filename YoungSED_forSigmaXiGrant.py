...
df = pd.read_csv('Data/young_comp/FIRE1139-3159 (M9gamma) SED.txt', sep=" ", comment='#', header=None,
                      names=["w", "f", "err"])
df2 = pd.read_csv('Data/young_comp/FIRE1139-3159 (M9gamma) phot.txt', sep=" ", comment='#', header=None,
                           names=["w", "f", "err"])
# Define regions of overlap
opt = df[(df['w'] <= 0.8315)]
overlap = df[(df['w'] >= 0.8315) & (df['w'] <= 0.99)]
nir = df[(df['w'] >= 0.8305) & (df['w'] <= 2.5)]
mir = df[(df['w'] >= 3)]
...
ax1.loglog(mir['w'], mir['f'], c='#007534')

# ----- Plot Photometric points -----
ax1.scatter(df2['w'][0], df2['f'][0],  c='#ae017e', s=150, zorder=6)  # 2MASS_H
ax1.scatter(df2['w'][1], df2['f'][1],  c='#ae017e', s=150, zorder=7)  # 2MASS_J
ax1.scatter(df2['w'][2], df2['f'][2],  c='#ae017e', s=150, zorder=6)  # 2MASS_K
ax1.scatter(df2['w'][3], df2['f'][3],  c='#dd3497', s=150, zorder=6)  # Denis_I
ax1.scatter(df2['w'][4], df2['f'][4],  c='#dd3497', s=150, zorder=6)  # Denis_J
ax1.scatter(df2['w'][5], df2['f'][5],  c='#3E0366', s=150, zorder=6)  # Irac_Ch4
...
ax1.annotate('IRS', xy=(1.7, 0.2*10**(-14)), color='#009B45', fontsize=15)
